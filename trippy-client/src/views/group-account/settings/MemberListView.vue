...
const filteredMembers = computed(() => {
  ...
  return memberStore.groupMembers.filter(
    (member) => member.userName && member.userName.includes(searchedMember.value),
  );
});
  ...
  <div class="h-full w-full">
    ...
    <div v-if="!searchedMember" class="px-2 mt-5">
      <div class="mb-4">
        ...
        <p class="body2">{{ leaderMember.userName }}</p>
      </div>
      <div v-if="!members.length == 0">
        <p class="caption1 text-gray-500 mb-1">멤버</p>
        <p class="body2 mb-1" v-for="member in members" :key="member.id">{{ member.userName }}</p>
      </div>
      <div v-else>
        <div>
          <p class="caption1 text-gray-500 mb-1">멤버</p>
